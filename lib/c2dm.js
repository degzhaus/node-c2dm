...
C2DM.prototype.login = function(cb) {
    var self = this;
    self.on('error', function(){console.log(" **** Error caught **** <1> ",arguments)})
    ...
    var request = https.request(this.loginOptions, function(res) {
        this.on('error', function(){console.log(" **** Error caught **** <2> ",arguments)})
        res.on('error', function(){console.log(" **** Error caught **** <3> ",arguments)})
        var data = '';
        function respond() {
    ...
    request.on('error', function(){console.log(" **** Error caught **** <4> ",arguments)})
    request.end(querystring.stringify(postData));
};

C2DM.prototype.send = function(packet, cb) {
    var self = this;
    self.on('error', function(){console.log(" **** Error caught **** <5> ",arguments)})
    ...
    operation.attempt(function(currentAttempt) {
        ...
        var request = https.request(self.c2dmOptions, function(res) {
            this.on('error', function(){console.log(" **** Error caught **** <6> ",arguments)})
            res.on('error', function(){console.log(" **** Error caught **** <7> ",arguments)})
            var data = '';
        ...
        request.on('error', function(){console.log(" **** Error caught **** <8> ",arguments)})
